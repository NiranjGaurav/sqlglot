...
class CommitterTask(Task):
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        if isinstance(retval, dict) and retval.get('success'):
            ...
            logger.info(f"✅ Committer task {task_id} completed: {committed_files} files committed for session {session_id}")
...
def commit_session_to_iceberg(
    ...
    try:
        ...
        if not validation_result["is_valid"]:
            logger.error(f"❌ Staged file validation failed for session {session_id}: "
        ...
        logger.info(f"✅ Iceberg commit successful for session {session_id}: "
        ...
        try:
            ...
            logger.warning(f"⚠️ Failed to update manifest for session {session_id}: {str(manifest_error)}")
        ...
        logger.error(f"❌ Critical error during Iceberg commit for session {session_id}: {error_msg}")
...
def retry_failed_commit(
    ...
    logger.info(f"🔄 Retrying Iceberg commit for session {session_id} (attempt {retry_attempt}/{max_retries})")
    ...
    try:
        ...
        if commit_result["success"]:
            ...
            logger.warning(f"⚠️ Retry attempt {retry_attempt} failed for session {session_id}: {commit_result.get('error')}")
            commit_result["retry_attempt"] = retry_attempt
        ...
        logger.error(f"❌ Error during retry attempt {retry_attempt} for session {session_id}: {str(e)}")
...
def commit_multiple_sessions(
    ...
    for i, session_id in enumerate(session_ids):
        try:
            ...
            if commit_result["success"]:
                ...
                results["summary"]["total_files_committed"] += commit_result.get("committed_files", 0)
                results["summary"]["total_rows_committed"] += commit_result.get("total_rows", 0)
            else:
                results["failed_commits"] += 1
                logger.warning(f"⚠️ Failed to commit session {session_id}: {commit_result.get('error')}")
    ...
    logger.info(f"✅ Multi-session commit complete: {results['successful_commits']} successful, "
