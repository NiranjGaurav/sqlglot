...
# AWS Configuration with IAM role support and environment variable fallback
...
# Check credential availability - support both IAM roles and environment variables
def _detect_credential_source():
    """Detect available credential source"""
...
if CREDENTIAL_SOURCE == "none":
    raise EnvironmentError("No AWS credentials available. Ensure either environment variables (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY) are set or IAM role is configured.")
...
def get_iceberg_catalog():
    ...
    if _iceberg_catalog is None:
        try:
            ...
            logger.info(f"Using credential source: {CREDENTIAL_SOURCE}")
            ...
            # Only add explicit credentials if using environment variables
            if CREDENTIAL_SOURCE == "environment":
                logger.info(f"Using explicit credentials: {AWS_ACCESS_KEY_ID[:10]}...")
                ...
                logger.info("Using IAM role credentials (automatic detection)")
...
def commit_staged_files_streaming(
    ...
    try:
        ...
        # Set up S3 filesystem with credential detection
        if CREDENTIAL_SOURCE == "environment":
