...
def process_query_batch(self, job_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    logger.warning("⚠️ process_query_batch is DEPRECATED. Use process_batch_to_staging_task for PyArrow-free processing")
...
from automated_processing.tasks_committer import commit_session_to_iceberg, retry_failed_commit, commit_multiple_sessions, CommitterTask
...
class WorkerTask(celery.Task):
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        if isinstance(retval, dict) and 'staged_file' in retval:
            logger.info(f"✅ Worker task {task_id} completed: {retval.get('processed_count', 0)} queries staged")
...
def process_batch_to_staging_impl(
    ...
    try:
        ...
        logger.info(f"🔄 Processing batch {batch_id} in session {session_id}: {num_queries:,} queries")
        ...
        for chunk_start in range(0, num_queries, chunk_size):
            ...
            for i, query_text in enumerate(chunk_queries):
                ...
                try:
                    ...
                    all_supported_functions_lists.append([str(f) for f in supported_funcs if f is not None])
                    
                    unsupported_funcs = analysis.get('unsupported_functions', []) or []
                    all_unsupported_functions_lists.append([str(f) for f in set(unsupported_funcs) if f is not None])
                    
                    unsupported_after_funcs = analysis.get('unsupported_functions_after_transpilation', []) or []
                    all_unsupported_functions_after_transpilation_lists.append([str(f) for f in unsupported_after_funcs if f is not None])
        ...
        if testing:
            ...
            for i in range(num_rows):
                query_results.append({
                    ...
                    'unsupported_functions_after_transpilation': all_unsupported_functions_after_transpilation_lists[i],
        ...
        logger.info(f"✅ Batch {batch_id} processing complete: {num_queries:,} processed, "
...
def create_staging_manifest_impl(
    ...
    try:
        ...
        logger.info(f"✅ Created staging manifest for session {session_id}: "
...
def process_batch_to_staging_task(self, session_id: str, batch_id: int, queries_list: list, metadata: dict, testing: bool = False) -> dict:
    """
...
def create_staging_manifest_task_wrapper(self, worker_results: list, session_id: str, session_metadata: dict) -> dict:
    """
    ...
    logger.info(f"    - worker_results: type={type(worker_results)}, value={worker_results[:3] if isinstance(worker_results, list) else worker_results}")
    logger.info(f"    - session_id: type={type(session_id)}, value={session_id}")
    logger.info(f"    - session_metadata: type={type(session_metadata)}, value={list(session_metadata.keys()) if isinstance(session_metadata, dict) else session_metadata}")
    
    # Validate parameters are in correct format
    if not isinstance(worker_results, list):
        logger.error(f"❌ worker_results should be list, got {type(worker_results)}: {worker_results}")
        return {
