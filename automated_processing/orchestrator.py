    ...
    try:
        ...
        if use_chord:
            ...
            try:
                ...
                r.hset(session_meta_key, 'company_name', session_metadata.get('company_name', 'unknown'))
                r.hset(session_meta_key, 'session_name', session_metadata.get('session_name', ''))
                r.hset(session_meta_key, 'orchestration_type', 'chord')
                r.hset(session_meta_key, 'from_dialect', session_metadata.get('from_dialect', ''))
                r.hset(session_meta_key, 'to_dialect', session_metadata.get('to_dialect', ''))
    ...
    try:
        ...
        if workers_status["status"] == "unknown":
            ...
            try:
                ...
                logger.info(f"🔍 Inferring worker status for {session_id}: Redis key exists = {r.exists(session_meta_key)}")
                    ...
                    if expected_batches:
                        ...
                        if current_files == 0:
                            ...
                            if staging_stats.get("staging_complete"):
                                workers_status = {"status": "completed", "inferred": True, "files_cleaned": True}
                            else:
                                workers_status = {"status": "processing", "inferred": True, "completed_tasks": 0, "total_tasks": expected_batches}
                            ...
                            if staging_stats.get("staging_complete"):
                                ...
                                workers_status = {
                                    "status": "running" if current_files < expected_batches else "completed",
                                    "inferred": True,
        ...
        if overall_status == "unknown":
            ...
            try:
                from .staging import read_staging_manifest
                manifest = read_staging_manifest(session_id)
                logger.info(f"📋 Manifest exists: {manifest is not None}, staging_complete: {manifest.get('staging_complete') if manifest else None}")
                ...
                if manifest and manifest.get("staging_complete"):
                    ...
                    if "metadata" in metadata:
                        ...
                        total_files = metadata.get("total_files", nested_metadata.get("total_files", 0))
                        logger.info(f"📈 Using nested metadata: successful_batches={successful_batches}, failed_batches_count={failed_batches_count}")
                    else:
                        # Use direct metadata
                        successful_batches = metadata.get("successful_batches", 0)
                        failed_batches_count = metadata.get("failed_batches_count", 0)
                        total_files = metadata.get("total_files", 0)
                        logger.info(f"📈 Using direct metadata: successful_batches={successful_batches}, failed_batches_count={failed_batches_count}")
                    
                    logger.info(f"🧮 Condition check: successful_batches > 0 = {successful_batches > 0}, failed_batches_count == 0 = {failed_batches_count == 0}")
                    if (successful_batches > 0 and failed_batches_count == 0):
                        ...
                        logger.info(f"💾 Iceberg commit data found: {iceberg_commit is not None and len(iceberg_commit) > 0}")
                        if iceberg_commit:
                            committer_status = {
                                "status": "SUCCESS",
                                "result": {
                                    ...
                                    "commit_duration_seconds": iceberg_commit.get("commit_duration_seconds", 0),
                                    "total_commit_duration_seconds": iceberg_commit.get("commit_duration_seconds", 0),
                        ...
                        logger.info(f"✅ Detected completed session {session_id} from manifest metadata: {successful_batches} batches, {iceberg_commit.get('total_rows', 0)} rows")
            except Exception as e:
        ...
        if (overall_status == "staged_ready_for_commit" and 
            committer_status.get("status") == "not_started" and
            CELERY_AVAILABLE and celery):
            ...
            try:
                ...
                manifest = read_staging_manifest(session_id)
                if manifest and manifest.get("iceberg_commit"):
                    logger.info(f"🛑 Session {session_id} already completed - skipping auto-trigger (manifest has commit data)")
                elif manifest and manifest.get("metadata", {}).get("iceberg_commit"):
                    logger.info(f"🛑 Session {session_id} already completed - skipping auto-trigger (nested commit data)")
                else:
                    # Safe to proceed with auto-trigger
                    logger.info(f"🚀 Auto-triggering committer for ready session {session_id}")
                    from worker import commit_session_to_iceberg_task
                    ...
                    try:
                        ...
                        if r.exists(session_meta_key):
                            session_metadata = {
                                'company_name': r.hget(session_meta_key, 'company_name').decode() if r.hget(session_meta_key, 'company_name') else 'auto_commit',
                                'from_dialect': r.hget(session_meta_key, 'from_dialect').decode() if r.hget(session_meta_key, 'from_dialect') else '',
                                'to_dialect': r.hget(session_meta_key, 'to_dialect').decode() if r.hget(session_meta_key, 'to_dialect') else ''
                            }
                        else:
                    ...
                    logger.info(f"✅ Auto-triggered committer task {committer_task.id} for session {session_id}")
        ...
        if staging_stats["total_files"] > 0 and overall_status not in ["completed", "failed"]:
            try:
                ...
                if (overall_status == "unknown" and validation_result and 
                    validation_result.get("valid_files", 0) == 0 and 
                    validation_result.get("invalid_files", 0) > 0):
                    all_key_not_exist = all(
                        "does not exist" in str(error.get("error", "")).lower()
                        for error in validation_result.get("errors", [])
                    )
                    if all_key_not_exist:
                        logger.info(f"✅ Detected completed session {session_id} - all {validation_result['invalid_files']} files were cleaned up after commit")
                        overall_status = "completed"
                        workers_status["status"] = "completed"
                                ...
                                for key in r.scan_iter(match=committer_key_pattern):
                                    try:
                                        ...
                                        if task_data:
                                            ...
                                            if (task_result.get("result", {}).get("session_id") == session_id and
                                                task_result.get("result", {}).get("total_rows")):
        ...
        if overall_status == "completed":
            try:
                ...
                if manifest and manifest.get("metadata"):
                    ...
                    if (committer_status.get("status") == "not_started" and 
                        metadata.get("iceberg_commit")):
                    ...
                    if (workers_status.get("completed_tasks", 0) == 0 and 
                        ...
                        if original_files > 0:
                            ...
                            if "metadata" in metadata:
                                ...
                                if total_queries > 0:
                                    ...
                                    staging_stats["total_size_bytes"] = int(estimated_size_mb * 1024 * 1024)
                    
                    logger.info(f"✅ Updated session details from manifest for completed session {session_id}")
            except Exception as e:
                logger.debug(f"Could not update session details from manifest for {session_id}: {e}")
