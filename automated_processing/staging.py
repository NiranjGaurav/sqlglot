...
def get_s3_filesystem() -> s3fs.S3FileSystem:
    """Get configured S3 filesystem using s3fs with credential detection"""
...
def write_native_data_to_staging(
    ...
    try:
        ...
        with s3fs.open(bucket_and_key, "wb") as f:
            ...
            pq.write_table(table, f, compression="snappy", write_statistics=True)
...
def list_staged_files(session_id: str) -> List[str]:
    ...
    try:
        ...
        logger.info(
            f"📂 Found {len(staged_files)} staged files for session {session_id} (prefix: {staging_prefix})"
        )
...
def read_staging_manifest(session_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug(
            f"📋 Read staging manifest for session {session_id}: {len(manifest_data.get('staged_files', []))} files"
        )
